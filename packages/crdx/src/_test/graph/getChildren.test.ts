...
import { buildGraph, findByPayload } from "../helpers/graph.js"
...
describe("getChildren", () => {
  const graph = buildGraph(`
         ...
         a ─ b ─┤         └─── f ───┤     ├─ n
                ...
                └───── j ─── k ── l ──────┘           
      `)
  test("root has 1 child", () =>
    expect(getChildren(graph, getRoot(graph))).toHaveLength(1))
  ...
  test("e has 1 child", () =>
    expect(getChildren(graph, findByPayload(graph, "e"))).toHaveLength(1))
