...
That is not hypothetical here: an ear anchored to "the earliest lockbox naming this invitation's signature key" was defeated by an author who read the key off the public invitation link and then wrote their own ear on a head from before the invitation existed. See `auth-6bw` for the backdating primitive itself.
...
- `keyMap`'s first-wins tie rule. It is no longer load-bearing — `selectors/keys` will not treat a keyset the graph never carried as current, and an admission can no longer introduce one — but it is order-dependent, and backdating is the thing that would reach it.
- `selectors/keys` resolving the current generation as the last entry in `keyHistory` that the device holds. Backdating can only place a keyset *earlier* in that list, which cannot make it current, so this one does not appear to be reachable that way — but it is order-shaped and is recorded here rather than argued away. `Team.rotateKeys` uses the list's *length*, which is a count and not an ordering.
