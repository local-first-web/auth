...
| `selectors/lockboxesInScope` | who gets a replacement when a scope rotates | **satisfies** — every recipient kind is anchored to something the graph carries. Members, servers and devices to the team's record of them; a ROLE to the role existing and the manifest carrying that role's current keyset from `keyHistory`; an EPHEMERAL ear to an invitation the team issued, and to the ear that invitation's own link posted (the earliest lockbox naming its signature key, which nobody can get in front of without the seed) |
